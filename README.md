...
2. 학교 이메일 기반 mock 회원가입 또는 로그인으로 진입 흐름을 보여준다.
3. 모집글 목록으로 이동해 필터를 적용하고 관심 글을 선택한다.
4. 상세 화면에서 팀 소개와 모집 역할을 확인한 뒤 mock 지원을 완료한다.
5. 이어서 새 모집글을 작성하고 생성된 상세 화면까지 시연한다.
...
- 인증 / 상태 관리: localStorage 기반 mock 로그인/회원가입 + 클라이언트 상태 중심
- 배포 방식: Vercel
