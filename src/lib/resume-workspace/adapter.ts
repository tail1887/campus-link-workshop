...
export function buildResumeWorkspaceViewModel(input: {
  ...
  if (
    ...
    return {
      ...
      notes: [
        ...
        "로그인 후에는 같은 이력서 draft 경계를 사용해 AI suggestion preview와 apply 흐름을 확인할 수 있습니다.",
      ],
  ...
  return {
    ...
    subtitle:
      authContext.user.role === "admin"
        ? "공통 Phase 2 계약을 유지한 채 own-record 편집 구조, completeness 계산, AI suggestion 흐름을 함께 확인할 수 있습니다."
        : "프로필 정보와 연결된 기본 이력서를 편집하고, completeness와 AI suggestion preview를 실시간으로 확인할 수 있습니다.",
    dataSource,
    sectionDefinitions: resumeSectionDefinitions,
    notes: [
      ...
      "AI suggestion 요청은 현재 draft를 resume contract로 직렬화해 /api/ai/suggestions/jobs에 보내고, 적용 결과는 다시 같은 draft 편집기에 반영합니다.",
    ],
    replacementPoints: resumeAiReplacementPoints,
